...
void run_gpu(unsigned char *host_image, const int width, const int height)
{
	...
	dim3 dimGridShared((int)ceil((float)(width)/5), (int)ceil((float)(height)/5));
	dim3 dimBlockShared(5, 5);
	...
	gpu_compute_xgrad<<<dimGrid, dimBlock, shared_mem>>>(device_image, width, height, device_x_component);
	gpu_compute_ygrad<<<dimGrid, dimBlock, shared_mem>>>(device_image, width, height, device_y_component);
	...
	gpu_gradient_matrix<<<dimGrid, dimBlock>>>(device_x_component, device_y_component, width, height, xx_grad, xy_grad, yy_grad);

	gpu_average_blur<<<dimGridShared, dimBlockShared, shared_mem>>>(xx_grad, width, height);
	gpu_average_blur<<<dimGridShared, dimBlockShared, shared_mem>>>(xy_grad, width, height);
	gpu_average_blur<<<dimGridShared, dimBlockShared, shared_mem>>>(yy_grad, width, height);


	//float *xx_grad_host = (float *)malloc(width * height * sizeof(float));

	gpu_harris_corner<<<dimGrid, dimBlock>>>(width, height, harris, xx_grad, xy_grad, yy_grad);

	gpu_nonmax_suppression<<<dimGrid, dimBlock>>>(width, height, harris);
	
	gpu_threshold<<<dimGrid, dimBlock>>>(device_img, width, height, harris);
	...
	cudaMemcpy(host_image, device_img, mem_size, cudaMemcpyDeviceToHost);
